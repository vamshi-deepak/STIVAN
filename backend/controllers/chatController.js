...
const handleChat = async (req, res, next) => {
  ...
  // If user asks 'what was my idea' or similar, return latest idea summary (context-aware)
  const asksWhatWasMyIdea = (
    /what\s+was\s+my\s+idea\??/.test(normalized) ||
    /tell\s+me\s+about\s+my\s+idea\??/.test(normalized) ||
    /do\s+you\s+remember\s+my\s+idea\??/.test(normalized)
  );
  if (asksWhatWasMyIdea && userId) {
    try {
      let idea = null;
      if (ideaId) idea = await Idea.findOne({ _id: ideaId, userId }).lean();
      if (!idea) idea = await Idea.findOne({ userId }).sort({ createdAt: -1 }).lean();
      if (idea) {
        const summary = `Your idea "${idea.title}" — ${idea.description || idea.fullExplanation || idea.summary || 'No detailed description saved.'}`;
        try { await ChatMessage.create({ user: userId, idea: idea._id, role: 'bot', text: summary, metadata: { type: 'idea_recall' } }); } catch (e) {}
        return res.json({ success: true, reply: summary });
      } else {
        const msg = 'I don\'t have any saved ideas for your account yet.';
        try { await ChatMessage.create({ user: userId, role: 'bot', text: msg, metadata: { type: 'idea_recall' } }); } catch (e) {}
        return res.json({ success: true, reply: msg });
      }
    } catch (e) {
      console.error('Error recalling idea:', e.message);
    }
  }
};
...
const clearUserChats = async (req, res) => {
  try {
    ...
    // Support deleting only chats for a specific idea if ideaId query param is provided
    const ideaId = req.query.ideaId || null;
    const filter = { user: userId };
    if (ideaId && ideaId !== 'general') {
      filter.idea = ideaId;
    }

    const result = await ChatMessage.deleteMany(filter);
    const msg = ideaId && ideaId !== 'general' ? 'Chats for idea cleared' : 'All chats cleared';
    res.json({ success: true, deletedCount: result.deletedCount || 0, message: msg });
  } catch (err) {
