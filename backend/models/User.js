...
userSchema.methods.generateOtp = function() {
  ...
  return this.updateOne({ $set: { otp, otpExpires: expiresAt } }).then(() => ({ otp, otpExpires: expiresAt }));
};

// Verify provided OTP; if valid, clear it and return true, else false
userSchema.methods.verifyOtp = async function(providedOtp) {
  ...
  await this.updateOne({ $unset: { otp: 1, otpExpires: 1 } });
  return true;
};
